...
trait HasError {
    ...
    public function getMissingDependencyError( $pluginName, $pluginUrl ) {
        return new \WP_Error( 'missing_dependency', sprintf(
            /* translators: 1: add-on name, 2: dependency plugin URL, 3: dependency plugin name */
            __( '<b>%1$s Plugin</b>: <a target="_blank" href="%2$s">%3$s</a> must be installed', 'wp-all-import' ),
            $this->name(), $pluginUrl, $pluginName
        ) );
    }
}
