...
function searchExistingInAttachedFile( $url, $post_id, $file_type, $importData, $logger ) {
    ...
    if ( empty( $attch ) ) {
        ...
        $logger and call_user_func( $logger, sprintf( __( '- Searching for existing image `%1$s` by `_wp_attached_file` `%2$s`...', 'wp-all-import' ), $url, $image_name ) );
        $attch = wp_all_import_get_image_from_gallery( $image_name, $targetDir, $file_type );
    }

    if ( ! empty( $attch ) ) {
        $logger and call_user_func( $logger, sprintf( __( '- Existing image was found by `_wp_attached_file` ...', 'wp-all-import' ), $url ) );
...
function searchExistingImage( $url, $post_id, $mode, $file_type, $import_data, $logger ) {
    ...
    switch ( $mode ) {
        case 'by_url':
            ...
            $logger and call_user_func( $logger, sprintf( __( '- Searching for existing image `%s` by URL...', 'wp-all-import' ), rawurldecode( $url ) ) );
            $attch = $imageList->getExistingImageByUrl( $url );

            if ( $attch ) {
                /* translators: %s: image URL */
                $logger and call_user_func( $logger, sprintf( __( 'Existing image was found by URL `%s`...', 'wp-all-import' ), $url ) );
        ...
        default:
            /* translators: %s: image URL */
            $logger and call_user_func( $logger, sprintf( __( '- Searching for existing image `%s` by Filename...', 'wp-all-import' ), rawurldecode( $url ) ) );
            $attch = $imageList->getExistingImageByFilename( basename( $url ) );

            if ( $attch ) {
                /* translators: %s: image filename */
                $logger and call_user_func( $logger, sprintf( __( 'Existing image was found by Filename `%s`...', 'wp-all-import' ), basename( $url ) ) );
