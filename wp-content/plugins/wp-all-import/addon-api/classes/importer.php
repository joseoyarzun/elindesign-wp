...
class PMXI_Addon_Importer {
    ...
    public function simplify( array $importData, array $parsedData ) {
        ...
        if ( $importData['logger'] ) {
            call_user_func( $importData['logger'], '<strong>' . sprintf(
                /* translators: %s: add-on name */
                __( '%s:', 'wp-all-import' ),
                strtoupper( $this->addon->name() )
            ) . '</strong>' );
        }

        if ( ! array_key_exists( $this->addon->slug, $options ) ) {
            call_user_func( $importData['logger'], __( 'No options found for this addon, skipping...', 'wp-all-import' ) );
        ...
        foreach ( $fields as $field_index => $field ) {
            ...
            if ( ! $this->canUpdateField( $field, $options ) ) {
	            ...
	            call_user_func( $importData['logger'], sprintf( __( '- Field `%s` skipped due to import settings.', 'wp-all-import' ), $field['key'] ) );
