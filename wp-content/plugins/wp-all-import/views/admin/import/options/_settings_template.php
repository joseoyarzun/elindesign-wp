...
<div class="wpallimport-collapsed closed wpallimport-section">
	<div class="wpallimport-content-section">
		<div class="wpallimport-collapsed-header">
			<h3><?php esc_html_e('Configure Advanced Settings','wp-all-import');?></h3>
		</div>
		<div class="wpallimport-collapsed-content" style="padding: 0;">
			<div class="wpallimport-collapsed-content-inner">				
				<table class="form-table" style="max-width:none;">
					<tr>
						<td colspan="3">
							<h4><?php esc_html_e('Import Speed Optimization', 'wp-all-import'); ?></h4>									
							<div class="input">
                                <label for="processing_iteration_logic_custom"><?php esc_html_e('In each iteration, process', 'wp-all-import');?></label> <input type="text" name="records_per_request" style="vertical-align:middle; font-size:11px; background:#fff !important; width: 40px; text-align:center;" value="<?php echo esc_attr($post['records_per_request']) ?>" /> <?php esc_html_e('records', 'wp-all-import'); ?>
                                <a href="#help" class="wpallimport-help" style="position: relative; top: -2px;" title="<?php esc_html_e('WP All Import must be able to process this many records in less than your server\'s timeout settings. If your import fails before completion, to troubleshoot you should lower this number. If you are importing images, especially high resolution images, high numbers here are probably a bad idea, since downloading the images can take lots of time - for example, 20 posts with 5 images each = 100 images. At 500Kb per image that\'s 50Mb that needs to be downloaded. Can your server download that before timing out? If not, the import will fail.', 'wp-all-import'); ?>">?</a>
                                ...
                                <div class="input" style="margin:5px 0px;">
                                    ...
                                    <label for="chuncking"><?php echo wp_kses( sprintf(
                                        /* translators: %s: chunk size (record limit) */
                                        __('Split file up into <strong>%s</strong> record chunks.', 'wp-all-import'),
                                        esc_html(PMXI_Plugin::getInstance()->getOption('large_feed_limit'))
                                    ), array('strong' => array()) ); ?></label>
                                    <a href="#help" class="wpallimport-help" style="position: relative; top: -2px;" title="<?php esc_html_e('This option will decrease the amount of slowdown experienced at the end of large imports. The slowdown is partially caused by the need for WP All Import to read deeper and deeper into the file on each successive iteration. Splitting the file into pieces means that, for example, instead of having to read 19000 records into a 20000 record file when importing the last 1000 records, WP All Import will just split it into 20 chunks, and then read the last chunk from the beginning.','wp-all-import'); ?>">?</a>
                                </div>
							</div>				
							<div class="input">
								...
								<label for="is_fast_mode"><?php esc_html_e('Increase speed by disabling do_action calls in wp_insert_post during import.', 'wp-all-import') ?> 
									<a href="#help" class="wpallimport-help" style="position: relative; top: -2px;" title="<?php esc_html_e('This option is for advanced users with knowledge of WordPress development. Your theme or plugins may require these calls when posts are created. Next action will be disabled: \'transition_post_status\', \'save_post\', \'pre_post_update\', \'add_attachment\', \'edit_attachment\', \'edit_post\', \'post_updated\', \'wp_insert_post\'. Verify your created posts work properly if you check this box.', 'wp-all-import') ?>">?</a></label>
							</div>					
							<?php if ( ! $this->isWizard ): ?>

								<?php if ( 'taxonomies' == $post['custom_type'] ):?>
									<h4><?php esc_html_e('Taxonomy Type', 'wp-all-import'); ?></h4>
									<p><?php esc_html_e('Editing this will change the taxonomy type of the taxonomies processed by this import. Re-run the import for the changes to take effect.', 'wp-all-import');?></p> <br>
								<?php else: ?>
									<h4><?php esc_html_e('Post Type', 'wp-all-import'); ?></h4>
									<p><?php esc_html_e('Editing this will change the post type of the posts processed by this import. Re-run the import for the changes to take effect.', 'wp-all-import');?></p> <br>
								<?php endif; ?>
								...
								<?php if ( 'taxonomies' == $post['custom_type'] ):?>
									<div class="wp_all_import_change_taxonomy_type">
										...
										<select id="taxonomy_to_import">
											<option value=""><?php esc_html_e('Select Taxonomy', 'wp-all-import'); ?></option>
                                ...
                                ?>
									...
									<?php if ( class_exists('WooCommerce') &&  ( ! class_exists('PMWI_Plugin') || class_exists('PMWI_Plugin') && PMWI_EDITION == 'free') ): ?>
										<div class="wpallimport-upgrade-notice" rel="shop_order">
											...
											<?php else: ?>
												<p><?php esc_html_e('The WooCommerce Add-On is Required to Import Orders', 'wp-all-import'); ?></p>
											<?php endif; ?>
										...
										<div class="wpallimport-upgrade-notice" rel="shop_coupon">
											...
											<?php else: ?>
												<p><?php esc_html_e('The WooCommerce Add-On is Required to Import Coupons', 'wp-all-import'); ?></p>
											<?php endif; ?>
											<a href="https://www.wpallimport.com/checkout/?edd_action=add_to_cart&download_id=5839961&edd_options%5Bprice_id%5D=1&discount=welcome-upgrade-169&utm_source=import-plugin-free&utm_medium=upgrade-notice&utm_campaign=import-wooco-coupons"><?php esc_html_e('Purchase the WooCommerce Add-On', 'wp-all-import');?></a>
										</div>
									<?php endif; ?>
								...
								<?php endif; ?>
								<h4><?php esc_html_e('XPath', 'wp-all-import'); ?></h4>
								<p><?php esc_html_e('Editing this can break your entire import. You will have to re-create it from scratch.', 'wp-all-import');?></p> <br>
								<div class="input">
								...
								<h4><?php esc_html_e('CSV Delimiter', 'wp-all-import'); ?></h4>
								<div class="input">
								...
								<?php endif; ?>
								<h4><?php esc_html_e('Downloads', 'wp-all-import'); ?></h4>

								<div class="input">
									<?php // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotValidated ?>
									<button class="button button-primary download_import_template" rel="<?php echo esc_url(add_query_arg(array('page' => 'pmxi-admin-manage', 'id' => intval(isset($_GET['id']) ? $_GET['id'] : ''), 'action' => 'get_template', '_wpnonce' => wp_create_nonce( '_wpnonce-download_template' )), $this->baseUrl)); ?>" style="background-image: none;"><?php esc_html_e('Import Template', 'wp-all-import'); ?></button>
									<?php // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotValidated ?>
									<button class="button button-primary download_import_bundle" rel="<?php echo esc_url(add_query_arg(array('page' => 'pmxi-admin-manage', 'id' => intval(isset($_GET['id']) ? $_GET['id'] : ''), 'action' => 'bundle', '_wpnonce' => wp_create_nonce( '_wpnonce-download_bundle' )), $this->baseUrl)); ?>" style="background-image: none;"><?php esc_html_e('Import Bundle', 'wp-all-import'); ?></button>
								</div>
							<?php endif; ?>
							<h4><?php esc_html_e('Other', 'wp-all-import'); ?></h4>
							<div class="input">
								...
								<label for="is_import_specified"><?php esc_html_e('Import only specified records', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php esc_html_e('Enter records or record ranges separated by commas, e.g. <b>1,5,7-10</b> would import the first, the fifth, and the seventh to tenth.', 'wp-all-import') ?>">?</a></label>
							...
							<?php if (isset($source_type) and in_array($source_type, array('ftp', 'file'))): ?>						
								<div class="input">
									...
									<label for="is_delete_source"><?php esc_html_e('Delete source XML file after importing', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php esc_html_e('This setting takes effect only when script has access rights to perform the action, e.g. file is not deleted when pulled via HTTP or delete permission is not granted to the user that script is executed under.', 'wp-all-import') ?>">?</a></label>
								</div>						
							<?php endif; ?>
							<?php if (class_exists('PMLC_Plugin')): // option is only valid when `WP Wizard Cloak` pluign is enabled ?>						
								<div class="input">
									...
									<label for="is_cloak"><?php esc_html_e('Auto-Cloak Links', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php echo esc_attr( sprintf(
										/* translators: %s: plugin name */
										__('Automatically process all links present in body of created post or page with %s plugin', 'wp-all-import'),
							...
							<div class="input">
								...
								<?php if ( PMXI_Plugin::getInstance()->getOption('force_stream_reader') ): ?>
									...
									<label for="xml_reader_engine"><?php esc_html_e('Use StreamReader instead of XMLReader to parse import file', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php esc_html_e('WP All Import is being forced to use Stream Reader for all imports. Go to WP All Import ▸ Settings to modify this setting.', 'wp-all-import'); ?>">?</a></label>
								<?php else : ?>
									...
									<label for="xml_reader_engine"><?php esc_html_e('Use StreamReader instead of XMLReader to parse import file', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php esc_html_e('XMLReader is much faster, but has a bug that sometimes prevents certain records from being imported with import files that contain special cases.', 'wp-all-import'); ?>">?</a></label>
								<?php endif; ?>
							</div>
							...
							<div class="input">
								...
								<label for="use_alternative_excel_processing"><?php esc_html_e('Use alternative Excel processing method', 'wp-all-import') ?> <a href="#help" class="wpallimport-help" style="position:relative; top:0;" title="<?php esc_html_e('Enable this option if you experience memory issues or errors when importing Excel files. This uses a different processing method that can handle problematic Excel files.', 'wp-all-import'); ?>">?</a></label>
							</div>
							
							<div class="input" style="margin-top: 15px;">
								<p><?php esc_html_e('Friendly Name','wp-all-import');?></p> <br>
								<div class="input">
