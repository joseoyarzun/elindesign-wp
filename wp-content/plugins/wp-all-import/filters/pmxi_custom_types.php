...
function pmxi_pmxi_custom_types($custom_types) {
    if ( class_exists('WooCommerce') ) {
        ...
        $custom_types['woo_reviews']->labels->name = __('WooCommerce Reviews', 'wp-all-import');
    }
	if ( class_exists('WooCommerce') && ! class_exists('PMWI_Plugin') ) {
		if ( ! empty($custom_types['product']) ) $custom_types['product']->labels->name = __('WooCommerce Products','wp-all-import');
		if ( ! empty($custom_types['shop_order']) ) $custom_types['shop_order']->labels->name = __('WooCommerce Orders','wp-all-import');
		if ( ! empty($custom_types['shop_coupon']) ) $custom_types['shop_coupon']->labels->name = __('WooCommerce Coupons','wp-all-import');
