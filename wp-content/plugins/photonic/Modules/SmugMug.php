...
class SmugMug extends OAuth1 implements Level_One_Module, Level_Two_Module {
	 ...
	 * @return string|array
	 */
	private function make_chained_calls($chained_calls, $smug_args, $short_code) {
	...
	public function get_config_object($count = 500, $levels = 5) {
		$base = [
			'expand' => [
				'HighlightImage' => [
					'expand' => [
...
/*		$config = [
	 ...
	 * @return Collection
	 */
	private function get_collection($node, $short_code, $indent = '', $level = 0) {
	 ...
	 * @return Album_List
	 */
	private function process_albums($albums, $indent, $album_filter, $short_code, $pages) {
	 ...
	 * @return string|array
	 */
	private function process_images($response, $short_code, $header) {
	...
	private function construct_custom_size($custom, $sizes) {
	...
	public function build_level_2_objects($albums, array $short_code, $album_filter = [], &$options = []) {
